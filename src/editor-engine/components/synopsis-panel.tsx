...
export function SynopsisPanel({ projectId, model, initialLogline = "", initialSynopsis = "" }: Props) {
  const [open, setOpen] = useState(!!(initialLogline || initialSynopsis))
  ...
  // Auto-generar si no hay sinopsis y hay contenido
  useEffect(() => {
    if (!initialLogline && !initialSynopsis && documentFountain.trim().length > 200 && !autoGenerated) {
